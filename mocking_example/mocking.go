...
func (c *Calculator) SomeComplexAggregationFunction(
...
func (e *V1InternalEndpoint) Retrieve(
	startDate, endDate time.Time,
	field string,
) ([]SingleItem, error) {
	convertedStartTime := startDate.Format("2006-02-01")
	convertedEndTime := endDate.Format("2006-02-01")

	rawResp, err := http.Get(
		fmt.Sprintf("http://example-data-server/api/data-archive/v1/retail?field=%v&start-date=%v&end-date=%v",
			field,
			convertedStartTime,
			convertedEndTime,
		),
...
type DataRetriever interface {
	Retrieve(startDate, endDate time.Time, field string) (RawItems, error)
}

type FakeDataRetriever struct{}

func (f *FakeDataRetriever) Retrieve(
	startDate, endDate time.Time,
	field string,
) ([]SingleItem, error) {
	if field == "receipts" {
...
func specialHandler(w http.ResponseWriter, r *http.Request) {
	...
	w.Write([]byte("the rest of function is not yet implemented"))
}
